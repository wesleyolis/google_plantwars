...
class PlanetWars {
 public:
  ...
  std::vector<Fleet> fleets_;
	int* planet_cost_matrix;

	void calculate_matrix()
	{
		int num_planets = planets_.size();
		planet_cost_matrix = new int [ num_planets * num_planets ] ;
		
		int* ptr_pcm_pos =  planet_cost_matrix;
		
		for( int r = 0; r < num_planets; r++)
		{
